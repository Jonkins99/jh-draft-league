...
node scripts/rename-team.mjs <alt> <neu> [--name "…"] [--logo x.png] [--dry]  # Team-ID samt aller Verweise umziehen
```

**Ab Saison 2 zieht `seed-teams.mjs` Teamfarbe und amtierenden Trainer des Franchise aus der Vorsaison nach** — beide gehören zum Verein, nicht zur Saison. Nachgezogen wird nur in ein leeres Feld, sonst überschriebe ein zweiter Lauf spätere Änderungen aus der Oberfläche. Von der Trainerhistorie wandert ausschließlich der amtierende Trainer mit, und zwar als „vor der Saison" im Amt: Amtszeiten zählen Spieltage der jeweiligen Saison, eine kopierte Historie stünde auf den falschen.

**Die Team-ID ist ein Fremdschlüssel, keine Beschriftung.** Ein Tippfehler im Slug lässt sich nur über `rename-team.mjs` geradeziehen (neues Dokument, Verweise in `drafts`/`schedules`/`results`/`awards`/`press`/`pressSessions`/`battleLogs` nachziehen, altes löschen) — und nur, solange die Saison noch nicht läuft. Wer einen Slug ändert, der schon eine Saison überdauert hat, trennt das Team von seiner Franchise-Historie; das Skript warnt davor.
...
- **Termine:** je Team und Spieltag ein Interview und eine Pressekonferenz, die Verteilung auf „vor/nach dem Spiel" wird deterministisch aus `teamId + day` gelost (kein Schreibzugriff, auf jedem Gerät gleich). Davor liegt einmalig die Auftaktrunde (`bonusRoundDay(season)`); Pressebetrieb beginnt mit `pressFromDay(season)`. **Der späte Start war eine Ausnahme der Saison 1** (Auftaktrunde vor Spieltag 8, davor keine Termine) — ab Saison 2 liefern beide Funktionen 1: die Auftaktrunde ist der Saisonstart, kein Spieltag fällt weg, und ein Aufsteiger hat sofort Termine. Beide Werte hängen an der Saison, nicht an einer Konstante. **„Nach dem Spiel" hängt an der Pressefreigabe** (`isPressReleased`), nicht am dritten Kampf — erst reden, wenn die Redaktion das Spiel gesehen hat. `slot.needsRelease` unterscheidet „Match noch offen" von „wartet nur auf die Freigabe".
- **Bausteine im Textkörper** (`press-tiles.mjs`): `[marktwert: …]`, `[verlauf: …]`, `[statistik: …]`, `[team: …]`, `[trainer: …]`, `[ergebnis: …]`, `[tabelle: …]`, `[video: …]`, `[bild: <URL>]`. Aufgelöst wird **beim Anzeigen** (`presseView.articleBody` -> `renderTiles`), nicht beim Speichern: so bleibt der Beitrag reiner Text ohne zusätzliche Whitelist, eine Marktwertkachel zeigt den heutigen Stand, und im Editor bleibt der Baustein als Zeile sichtbar. `TILE_MENU` ist dieselbe Liste beschriftet und trägt die Editor-Leiste.
- **Ein Baustein muss NICHT allein im Absatz stehen.** `findTiles` sucht statt zu prüfen; `resolveBlock` trennt den Absatz an der Fundstelle auf (Text davor, Kachel, Text danach) — ein Listenpunkt wird dabei nicht geteilt, dort hängt die Kachel hinten an. Ein Baustein, der ins Leere zeigt, verliert seine Klammer statt sie zu zeigen. Nur der INNERSTE Block wird aufgeteilt: der `textContent` eines Elternblocks enthält auch den Text der Kinder, die Fundstellen zeigten dort auf die falsche Stelle.
- **Schlüssel werden über `normKey` verglichen**, nicht über Gleichheit: ohne Diakritika, ohne Trennzeichen, klein. `findMon` nimmt zusätzlich den englischen Namen und einen EINDEUTIGEN Namensanfang, `findTeam` den Franchise-Slug ohne Saisonpräfix (Saison-Teams gehen vor), `findResult` die Match-Id ohne Präfix. Zwei Treffer sind kein Treffer — geraten wird nicht.
- **Freitext-Tabelle in Pipe-Schreibweise**: aufeinanderfolgende Zeilen `| Zelle | Zelle |` werden zu einer Tabelle (`parsePipeTable`/`pipeTableHtml`), `| --- | ---: |` setzt die Ausrichtung, `**Text**` hebt eine Zelle hervor. Bewusst Text und kein `<table>` im Textkörper: so übersteht die Tabelle den Sanitizer (der unbekannte Elemente auflöst), bleibt im Editor zeilenweise bearbeitbar und lässt sich ohne Tabellenwerkzeug tippen. Die Zeilen trennt entweder ein eigener Absatz oder ein `<br>`.
- **`plainText` räumt Bausteine und Tabellenzeilen weg** (`stripTiles`) — Vorschautext, Lesedauer und Suche zeigten sonst das Klammerwerk.
