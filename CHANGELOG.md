...
- **Teams for season 2.** Toxicroak Rangers and AC Arboliva are relegated; both belonged to Henrik, which is exactly the case where the draft order cannot be computed and waits for him to choose who picks seventh. The seed script refuses to write a placeholder: a team written under the wrong slug could not be corrected later, because the eternal table, the contract extensions and the market history all hang off that slug.
- **Colour and coaching staff belong to the club, not to the season.** A franchise that survives relegation used to arrive in the new season blank: no team colour, no trainer on the bench. `seed-teams.mjs` now carries both over from the previous season, keyed on the franchise slug — but only into an empty field, so a second run never overwrites what was changed in the app since. Only the trainer actually in office moves along, and he starts the new season as "before the season": tenures are counted in matchdays of their own season, so a copied history would sit on the wrong ones.
- **Fixed: "Terestal Palace" was spelled with an e.** The club is Terastal Palace. Renaming it meant moving the document, because the team ID is a foreign key, not a label — `scripts/rename-team.mjs` writes the new ID, pulls every reference in schedule, results, draft, awards, press and battle logs along with it, and deletes the old one. It warns when a slug that already survived a season is about to be changed, since that would cut the club off from its own history.
- **Trainer seeding knows about seasons.** With two seasons the same club name exists twice; `seed-trainers.mjs --season 2` targets one season's teams and its own CSV.
- **Fixed: both Servol forms had no market value.** The sheet writes "Servol ♀"/"Servol ♂", the master data "Servol (weiblich)"/"(männlich)" — the alias table was missing them.

## Press
- **Fixed: a tile inside a sentence stayed raw text.** "[statistik: Mega-Floette]" was only resolved when it sat alone in its paragraph — which the desk, every prompt rule notwithstanding, regularly failed to do. Tiles are now searched for rather than checked: wherever one turns up, the paragraph is split at that spot — text before, tile, text after — and in a list item the bracket leaves the sentence and the tile hangs off the end of the point. A key that points nowhere loses its brackets instead of printing them. Because tiles are resolved when an article is displayed, all of this applies retroactively to every article already filed. The key itself may now be written loosely: accents, spaces instead of hyphens, a trailing full stop, the English name, a club without its season prefix, a match as "d3-m0" — all of it finds its row. Two candidates are still no candidate; nothing is guessed.
- **Every tile can be placed by hand.** The editor grew a row of tile buttons. Pick one and you get a searchable list of the keys that actually exist — Pokémon with their tier, clubs with their player, matches with their matchday — and the tile goes in as its own paragraph. Guessing a key from memory was the only way before, and a typo in one stayed invisible until the article was published.
- **A table for hand-written articles.** Rows in the form `| cell | cell |` become a real table; `| --- | ---: |` sets the alignment per column and `**text**` highlights a cell. The rows are plain text on purpose — so they survive the sanitizer, stay editable line by line and can be typed without a table tool. The toolbar drops in a skeleton. Tile brackets and table rows are kept out of the preview text, the reading time and the search, where they were pure noise.
- **A championship, a relegation and a safe mid-table are arithmetic, not a feeling.** The table metadata now carry, per team, the best and worst place still reachable plus three plain yes/no fields: title secured, safe from relegation, relegated for certain. The canon makes them binding — the newsroom no longer declares survival while a chaser can still pass on the last matchday.
- **More tiles, in every kind of article.** Alongside market value, club, trainer, result and video there are now a market-value curve over the season, a Pokémon's record (kills, deaths, K/D, appearances, win and survival rate), a table excerpt around a team, and an image. Two or three tiles per piece is the normal case now, not the exception — match report, free article, follow-up to an interview alike.
- **An image in the brief becomes an image in the article.** A commissioned piece with an image URL in its brief used to leave the bare address sitting in the running text. The address is now pulled out and placed as an image tile; only http(s) links to an actual image file are accepted.
- **Fewer house phrases.** The desk had settled into a handful of turns — spotlights that still glow, "the S-tier precious metal", the same opening every time. Those are banned by name now, a tier is a grading and not a title of nobility, and every request draws two binding form constraints (no metaphor from light or from war, no superlative, no sentence over twenty words, first paragraph without adjectives, …) plus one of twenty openings instead of nine.
- **The newsroom opens with matchday 1 — the late start was season 1 only.** The press joined that season halfway through, which is why its opening round sat before matchday 8 and the matchdays before it stayed empty. From season 2 on the opening round is the start of the season itself: every club, promoted ones included, has its interview and its press conference before a single battle is fought, and no matchday is skipped. The start day is read per season now, so nothing has to be touched again next year.
- **Talking comes after the report.** Interviews and press conferences after a match unlock with the press release, not with the third battle entered.
- **Fixed: second-division pieces were filed as first division.** "First division" attaches itself to every AI piece; it is now dropped as soon as a piece is explicitly in the second.
- **Fixed: "Redaktion" was printed twice** on the players' own articles. It is the origin, not a rubric — the stamp hangs on the article, and the rubric is gone.

## Awards & hall of fame
- **A tier award grades by the season it belongs to.** Once the pool was re-graded for season 2, "Best C tier" of season 1 offered the season-2 classes — Hisui-Tornupto had climbed to A in the meantime and was missing from the C vote it actually belonged in. The tier now comes from the squads of that season, which carry the class a Pokémon was drafted or signed with; the master data only fill in what no squad knows. The candidate list for a season award is cut to that season's squads as well, so clubs of another season no longer leak into it.
- **An unwatched ceremony moves to the top.** Finished votes whose ceremony you have not seen yet are sorted ahead of the rest under "Vergeben", and the tab carries a small pulsing dot for as long as one is waiting. Everything below it is archive.
- **Nobody starts the vote for the other one.** The choice between "start the vote" and "wait for the other" is gone. You either park your nominations and keep editing them, or you say you are done. When both are done, the vote begins by itself.
- **The hall of fame waits for the awards.** The season finale only unlocks once no vote is still open; until then the teaser says how many are missing. The box itself now stacks on a phone instead of squeezing the heading into a column.
- **The four newer ceremonies were a step back — they aren't any more.** The flip board left a black seam lying across the Pokémon, the slam blurred the plate beyond recognition and shook the whole stage, the approach flew in from far outside the frame and the gala burned out white. Each keeps its character at a calmer scale, and the stage no longer overflows, so the scrollbars that appeared and vanished during every reveal — and shifted the layout each time — are gone.

## Teambuilder & tables
- **The battle log can be written in the Teambuilder too**, not only in the result entry. Both use the same editor; each player only ever writes their own section, so two open windows cannot overwrite each other.
- **A stored set loads into the damage calculator by itself** when you pick that Pokémon — no button to hunt for. Only on a change of Pokémon, so your own entries stay put.
- **"0+" is gone from the default speed tiers.** Zero points is the "not invested" assumption; a speed-boosting nature on top of it is a combination nobody plans. It appears when it was actually entered.
- **"Init ← Tierlist" now reads "Init ← Speed Tier"** — the table is called Speed Tier everywhere else.
- **The placement chart drops the club crests.** Every club has its own colour now; the crests only covered their own lines. The legend gained a colour bar.
- **The table's placement numbers are all grey.** Gold and silver for the top three duplicated what the rows already say.
- **Fixed: squad sprites sat at the top of their tile** instead of in the middle. The award-pin slot counted as a grid row even when empty and pushed the sprite up.
## Draft plan (new)
- **A second tab in the draft view: plan before you pick.** Every one of your own teams gets its own plan, ten empty shells — two per tier. You load as many candidates into a slot as you seriously consider, not just one. Everything stays on the device and rides along, encrypted, in the private backup next to the Teambuilder data; nobody else ever sees it.
- **Paths, not permutations.** A path picks at most one candidate per slot. A path can branch off another one at any slot: the branch inherits everything up to the split and goes its own way from there. Correct something in the main plan and it travels into every branch that has nothing of its own there — which is why "these two differ only from slot 7 on" stays one line of data instead of a second full plan. Branches are listed indented under their parent, in a priority order you set yourself, with the split spelled out ("from B1: Mamoswine instead of Azumarill").
- **The order of the slots is its own decision.** Nobody picks S1, S2, A1, A2 in that order — an important C tier goes in round one. Each slot carries the position at which you intend to take it, movable up and down, per plan and, if you want, per path.
- **All paths at once, as thin lines.** The "Pfadlinien" layout puts one column per slot next to each other and draws every path as a line through its candidates, in its own colour. Dashed means: no longer reachable. Dense fields are offset against each other so crossing paths stay apart, and the whole board scrolls sideways on a phone.
- **The plan keeps itself.** It knows the draft only as stock: whoever already sits in a squad is no longer an option. A candidate another team has taken is struck through and hidden, a tier in which you already hold two is cleared out, and a path that can no longer be walked — because a wish is gone or because you picked outside it yourself — leaves the main list and moves into a collapsible section that names what killed it and jumps straight back to that slot to choose a replacement.
- **Every path can be weighed.** Type profile per attacking type, with the threat weighted — four times hurts more than twice, and a hole three Pokémon fall into weighs more than three separate ones. Initiative as pins in the spectrum of the whole pool, so a whole unoccupied speed range is visible at a glance. And suggestions computed from the actual gain: threat value before minus after, restricted to Pokémon that are still free and fit an open tier — so it is not the Pokémon with the most resistances that wins, but the one whose resistances sit where this path bleeds.
- **`draftplan-entwurf.html` runs the whole thing with an invented league** and the real Pokémon pool: start a draft, let an opponent snipe one of your candidates, put yourself on the clock.
